...
async function createRepo(): Promise<TursoRepository> {
...
describe("entries", () => {
  ...
  describe("updateEntry", () => {
    ...
    it("updates end_time and computes duration", async () => {
      ...
      expect(updated.duration_seconds).toBe(5400);
    });
  ...
  describe("listEntries", () => {
    ...
    it("orders by start_time descending", async () => {
      await seedEntries();
      const result = await repo.listEntries({});
      expect(result.entries[0]!.id).toBe("ent_3");
      expect(result.entries[2]!.id).toBe("ent_1");
    });
...
describe("sync", () => {
  it("returns a SyncResult", async () => {
    // In-memory client has no syncUrl — sync() will fail gracefully
