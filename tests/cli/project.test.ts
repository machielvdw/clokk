...
describe("clokk project list", () => {
  ...
  it("lists including archived", () => {
    ...
    const withArchived = parseJson(runClokk(["project", "list", "--archived", "--json"]).stdout);
    const withoutArchived = parseJson(runClokk(["project", "list", "--json"]).stdout);
...
describe("clokk project edit", () => {
  it("edits a project", () => {
    const r = runClokk(["project", "edit", "Acme Corp", "--client", "Acme Inc", "--json"]);
    expect(r.exitCode).toBe(0);
    const json = parseJson(r.stdout);
