...
export function registerTimerTools(server: McpServer, repo: Repository): void {
  ...
  server.registerTool(
    "stop_timer",
    {
      ...
      inputSchema: {
        description: z.string().optional().describe("Update the description before stopping"),
        tags: z.array(z.string()).optional().describe("Update tags before stopping"),
      },
      annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false },
    },
    async (input) =>
  ...
  server.registerTool(
    "switch_timer",
    {
      ...
      inputSchema: {
        ...
        tags: z.array(z.string()).optional().describe("Tags for the new timer"),
      },
      annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false },
    },
    async (input) =>
  ...
  server.registerTool(
    ...
    async (input) => handleToolCall(() => resumeTimer(repo, { id: input.id })),
  );
